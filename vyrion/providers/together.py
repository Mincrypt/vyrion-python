    ...
    def _get_client(self):
        if not self._client:
            try:
                ...
                raise ImportError(
                    "The 'openai' package is required for TogetherProvider. "
                    "Install it using 'pip install vyrion[openai]'"
                )
            self._client = openai.AsyncOpenAI(
                api_key=self.config.get("api_key"),
                base_url=self.config.get("base_url") or "https://api.together.xyz/v1",
        ...
        for part in content:
            if part.type == "text":
                ...
                if mime.startswith("text/") or mime in ("application/json", "text/csv"):
                    ...
                    raise ValueError(
                        f"Together does not natively support '{mime}' file attachments."
                    )
        return mapped
